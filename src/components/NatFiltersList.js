...
import '../CSS/nat-filter-list.less'
...
function NatFiltersList() {
    ...
    return (
        <ul className= 'list'>
            {nations.map((nation) => (
            <li className={currentNationality === nation.code ? `nat-${themeMode} active-${themeMode}` : `nat-${themeMode}` }
                key={nation.code}
                onClick={() => { dispatch(getNatId(nation.code)) }}> 
                <i className={`checkbox-${themeMode}`}>
                    {(currentNationality === nation.code) ? <ImCheckboxChecked/> : <ImCheckboxUnchecked/>}
                </i>
